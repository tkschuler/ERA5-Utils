...
# Define file names with MONTH-YEAR prefix
TQ_ML_FILE="${data_dir}${YEAR}-${MONTH}-${DAY}-tq_ml.grib"
#TQ_ML_FILE="${data_dir}test.txt"
ZLNSP_ML_FILE="${data_dir}${YEAR}-${MONTH}-${DAY}-zlnsp_ml.grib"
Z_FILE="${data_dir}${YEAR}-${MONTH}-${DAY}-z.grib"
LNSP_FILE="${data_dir}${YEAR}-${MONTH}-${DAY}-lnsp.grib"
MERGED_FILE="${data_dir}${YEAR}-${MONTH}-${DAY}-merged.grib"
PRES_TEMP_FILE="${data_dir}${YEAR}-${MONTH}-${DAY}_pres_temp.grib"
PRES_FILE="${data_dir}${YEAR}-${MONTH}-${DAY}_pres.grib"



# Define Python scripts
DOWNLOAD_SCRIPT="download_forecast.py"
COMPUTE_GEOPOTENTIAL_SCRIPT="tristan_compute_geopotential_on_ml.py"
...
python3 $DOWNLOAD_SCRIPT --year $YEAR --month $MONTH --day $DAY
