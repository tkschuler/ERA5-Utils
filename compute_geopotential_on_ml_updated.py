...
modified:       Cristian Simarro (20/03/2017) - migrated to eccodes
                ...
                Tristan schuler  (01/02/2025) - Bug fix for multiple levels
...
def main():
    ...
    print("DEBUG: Checking if input files exist in Python...")
    import os
    print(f"Checking {args.t_q}: {'Exists' if os.path.exists(args.t_q) else 'NOT FOUND'}")
    print(f"Checking {args.z_lnsp}: {'Exists' if os.path.exists(args.z_lnsp) else 'NOT FOUND'}")

    if not os.path.exists(args.t_q) or not os.path.exists(args.z_lnsp):
        print("ERROR: One or more input files do not exist. Check paths.")
        sys.exit(1)
    ...
    for date in codes_index_get(idx, 'date'):
        ...
        for time in codes_index_get(idx, 'time'):
            ...
            values = None  # Reset for each time step
